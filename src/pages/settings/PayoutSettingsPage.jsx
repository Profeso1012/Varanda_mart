...
export default function PayoutSettingsPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      const { data } = await sellerApi.post('/supplier/profile/bank-account/verify-account', {
  ...
  const handleRegister = async () => {
    ...
    try {
      const { data } = await sellerApi.post('/supplier/profile/bank-account', {
        bankCode: form.bankCode,
        accountNumber: form.accountNumber,
      ...
      setBankAccount({
        ...
        accountNumber: data.data.maskedAccount,
        accountName: data.data.accountName,
        recipientCode: data.data.recipientCode,
      });
      ...
      <div className="store-settings-page">
        <div className="store-settings-container">
          <div className="store-settings-header">
            <div>
              <h1 className="store-settings-title">Payout Settings</h1>
              <p className="store-settings-subtitle">Manage your bank account for supplier payouts</p>
            </div>
          </div>
          ...
          {bankAccount && (
            <div className="form-section mb-8 p-6 bg-white border border-gray-200 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="form-section-title mb-2">Active Payout Account</h3>
                  <p className="text-sm text-gray-600 mb-3">Your account is set up and ready to receive payouts</p>
                  <div className="space-y-2 text-sm">
                    ...
                    {bankAccount.recipientCode && (
                      <p><span className="font-medium">Recipient Code:</span> {bankAccount.recipientCode}</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          {!bankAccount && (
            <div className="form-section">
              <h2 className="form-section-title">Add Payout Account</h2>

              <p className="text-sm text-gray-600 mb-6">
                Set up your bank account to receive payments for your supplies. We'll verify the account and enable automatic transfers.
              </p>
