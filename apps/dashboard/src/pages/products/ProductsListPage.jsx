...
import { getCategoryName, getProductImageUrl, getProductPrice, getStockTotal } from '../../lib/productFields';
import { deleteLocalProduct, duplicateLocalProduct, getLocalProducts } from '../../lib/localCatalog';
...
function AddProductModal({ onClose, onChoose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1F2A30]/20 px-4 backdrop-blur-sm" onClick={onClose}>
      <div className="relative max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white p-6 shadow-2xl sm:p-8" onClick={(e) => e.stopPropagation()}>
        <button
          type="button"
          onClick={onClose}
...
function ActionButtons({ product, onEdit, onDelete, onDuplicate }) {
  return (
    <div className="flex items-center gap-3">
      <button type="button" onClick={() => onEdit(product)} className="text-[#D9573F] hover:text-[#B53F2D]" aria-label={`Edit ${product.name}`}>
        <Pencil size={18} />
      </button>
      <button type="button" onClick={() => onDuplicate(product)} className="text-[#321CFF] hover:text-[#2110B8]" aria-label={`Duplicate ${product.name}`}>
        <Copy size={18} />
      </button>
      <button type="button" onClick={() => onDelete(product)} className="text-[#1F2A30] hover:text-[#E32323]" aria-label={`Delete ${product.name}`}>
        <Trash2 size={18} />
      </button>
    </div>
  );
...
export default function ProductsListPage({ mode = 'seller' }) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const remoteProducts = data.data.products || [];
      const localProducts = mode === 'seller' ? getLocalProducts() : [];
      const merged = [
        ...localProducts.filter((local) => !remoteProducts.some((remote) => remote.id === local.id)),
        ...remoteProducts,
      ];
      setProducts(merged);
    } catch {
      setProducts(mode === 'seller' ? getLocalProducts() : []);
    } finally {
      setLoading(false);
    }
  }, [activeTab, endpoint, mode, perPage, search]);
  ...
  const handleDuplicate = async (p) => {
    if (mode === 'supplier') {
      alert('Duplicate is not available for supplier products yet.');
      return;
    }
    if (p._local || String(p.id).startsWith('local-product-')) {
      const copy = duplicateLocalProduct(p);
  ...
  const handleDelete = async (p) => {
    ...
    try {
      if (mode === 'supplier' && p.status === 'ACTIVE') {
        await sellerApi.post(`/supplier/products/${p.id}/pause`);
      }
  ...
  const getStatusLabel = (product) => {
    if (getStockTotal(product) === 0) return 'Out of Stock';
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-gray-100 bg-white px-4 py-4 sm:px-6">
      ...
      <div className="min-h-full bg-gray-50 p-4 sm:p-6">
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-100 bg-white w-full">
          <table className="w-full text-sm" style={{ minWidth: '800px' }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-8 sm:py-12 text-center">
                    <div className="mx-auto flex max-w-sm flex-col items-center px-4">
                      <img src="/empty_box.png" alt="Empty" className="h-24 sm:h-32 w-auto max-w-full object-contain" />
                      <p className="text-sm sm:text-base font-bold text-[#1F2A30] mt-3 sm:mt-4">No Products Yet</p>
                      <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-[#5C5D86]">Start adding your first product</p>
                      <button
                        type="button"
                        onClick={() => mode === 'seller' ? setShowAddModal(true) : navigate('/supplier/products/new')}
                        className="mt-4 sm:mt-6 flex items-center justify-center gap-2 rounded-full bg-[#22925B] px-5 py-2.5 sm:px-6 sm:py-3 text-xs sm:text-sm font-semibold text-white hover:bg-[#1a7a4a] w-full sm:w-auto"
                      >
                ...
                products
                  .filter((product) => activeTab === 'All' || product.status === activeTab.toUpperCase())
                  .filter((product) => !search || product.name?.toLowerCase().includes(search.toLowerCase()))
                  .map((product) => (
                  <tr key={product.id} onClick={() => handleEdit(product)} className="cursor-pointer border-b border-gray-50 transition-colors hover:bg-gray-50">
                    <td className="py-3 px-6">
                      <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 shrink-0">
                        {getProductImageUrl(product) ? <img src={getProductImageUrl(product)} alt={product.name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-200" />}
                      </div>
                    </td>
                    <td className="py-3 px-4 text-[#1F2A30] font-medium">{product.name}</td>
                    <td className="py-3 px-4 text-[#5C5D86]">{getCategoryName(product)}</td>
                    <td className="py-3 px-4 text-[#1F2A30]">{formatPrice(getProductPrice(product))}</td>
                    ...
                    <td className="py-3 px-4" onClick={(e) => e.stopPropagation()}>
                      <ActionButtons product={product} onEdit={handleEdit} onDelete={handleDelete} onDuplicate={handleDuplicate} />
                    </td>
                  </tr>
                ))
              )}
