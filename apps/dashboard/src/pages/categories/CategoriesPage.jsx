...
export default function CategoriesPage() {
  ...
  const deleteCategory = async () => {
    ...
    try {
      await sellerApi.delete(`/catalog/categories/${deleteModal.category.id}`, { 
        data: { reassignTo: null } 
      });
      await loadCategories();
      
      // Reset form if deleted category was selected
      if (selectedId === deleteModal.category.id) {
        resetForNew();
      }
  ...
  return (
    <DashboardLayout
      ...
      mode="seller"
      actions={
        <button onClick={resetForNew} className="flex items-center justify-center gap-2 rounded-full bg-[#22925B] w-10 h-10 sm:w-auto sm:h-auto sm:px-5 sm:py-2.5 text-sm font-semibold text-white hover:bg-[#1a7a4a]">
          <Plus size={16} />
    ...
    >
                ...
                <div key={category.id} className={`rounded-lg p-3 transition-colors ${isSelected ? 'bg-green-50' : 'hover:bg-gray-50'}`}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      ...
                      <span className="text-left text-sm font-bold text-[#1F2A30] truncate">
                        {category.name}
                      </span>
                    </div>
      ...
      <ConfirmDeleteModal
        ...
        message="Are you sure you want to delete this category? All products in this category will be uncategorized."
